...
prod_deploys = pipelines.filter(F.col("is_production").isin("true", "True", True))
...
weekly = pipelines.withColumn(
    ...
    F.sum(F.when(F.col("is_production").isin("true", "True", True),1).otherwise(0)).alias("deployment_count"),
).orderBy("week")
...
# Par projet (Jira)
by_project = tickets.groupBy("ProjectKey").agg(
    F.count("TicketKey").alias("nb_tickets"),
